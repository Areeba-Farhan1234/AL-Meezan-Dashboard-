...
const Balance = () => {
  ...
  const [timeline, setTimeline] = useState('monthly');
  const [open, setOpen] = useState(false);
  const shadow = theme.customShadows?.[0] ?? '0px 4px 12px rgba(0,0,0,0.1)';
  ...
  return (
    <Paper sx={{ p: 3, height: { xs: 500, sm: 355 } }}>
      <Stack
        direction="row"
        alignItems="center"
      ...
      <Stack mt={3} spacing={3} direction={{ xs: 'column', sm: 'row' }} alignItems="center">
        {balance.map((item) => (
          <Box
            key={item.id}
            px={2.5}
