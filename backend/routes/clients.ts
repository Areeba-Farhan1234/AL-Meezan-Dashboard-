...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    // Agar _id aa raha hai to hata dein
...
// ====== POST Save Revenue for a client ======
router.post('/:id/revenue', async (req: Request, res: Response) => {
  ...
  try {
    ...
    client.revenue = revenue; // overwrite revenue history
    await client.save();

    res.status(200).json({ message: 'Revenue saved successfully', revenue: client.revenue });
  } catch (err: any) {
    console.error('Error saving revenue:', err);
    res.status(500).json({ error: 'Failed to save revenue', details: err.message });
  }
});
